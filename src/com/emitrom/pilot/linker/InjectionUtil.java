...
public class InjectionUtil {
    ...
    private static String generateScriptInjector(String scriptUrl) {
        String value = _generateScriptInjector(scriptUrl);
        return ScriptInjectionUtils.fixScriptInjector(scriptUrl, value);
    }

    private static String _generateScriptInjector(String scriptUrl) {
        if (isRelativeURL(scriptUrl)) {
            return "  if (!__gwt_scriptsLoaded['" + scriptUrl + "']) {\n" + "    __gwt_scriptsLoaded['" + scriptUrl
                            + "'] = true;\n"
                            + "    document.write('<script language=\\\"javascript\\\" src=\\\"'+base+'" + scriptUrl
                            + "\\\"></script>');\n" + "  }\n";
        } else {
            return "  if (!__gwt_scriptsLoaded['" + scriptUrl + "']) {\n" + "    __gwt_scriptsLoaded['" + scriptUrl
