...
public class StackTraceCreator {
    ...
    static class Collector {
     ...
     if( arguments.callee.caller){
      ...
      return toReturn;
        ...
        /**
         * Attempt to infer the stack from an unknown JavaScriptObject that had
         * been thrown. The default implementation just returns an empty array.
         * 
         * @param e
         *            a JavaScriptObject
         */
    ...
    static class CollectorEmulated extends Collector {
        ...
        /**
         * When compiler.stackMode = emulated, return an empty string, rather
         * than a list of properties, since the additional information regarding
         * the origin of the JavaScriptException, relative to compiled
         * JavaScript source code, adds no real value, since we have fully
    ...
    static class CollectorChrome extends CollectorMoz {
        ...
        public JsArrayString inferFrom(JavaScriptObject e) {
            ...
            if (stack.length() == 0) {
                ...
                // Chrome contains the error itself as the first line of the
                // stack:
    ...
    /**
     * Subclass that forces reported line numbers to -1 (fetch from symbolMap)
    ...
    static String extractNameFromToString(String fnToString) {
        ...
        if (index == -1) {
            // Firefox 14 does not include parenthesis and uses '@' symbol
            // instead to terminate symbol
            index = fnToString.indexOf('@');
            /**
             * Firefox 14 doesn't return strings like 'function()' for anonymous
             * methods, so we assert a space must trail 'function' keyword for a
             * method named 'functionName', e.g. functionName:file.js:2 won't
             * accidentally strip off the 'function' prefix which is part of the
             * name.
             */
