...
class HandlerEvaluator {
  ...
  private boolean classImplementsInterface(JClassType j, JClassType jInterface) {
	  for (JClassType i : j.getImplementedInterfaces()) {
		  if (i == jInterface) {
			  return true;
		  }
	  }
    ...
    for (JMethod method : objectType.getInheritableMethods()) {
      ...
      if (method.getReturnType() == handlerRegistrationJClass
    		  || (method.getReturnType().isClassOrInterface() != null
    				  && classImplementsInterface(method.getReturnType().isClassOrInterface(), handlerRegistrationJClass))) {
