/************************************************************************
  JsonpRequest.java is part of Ti4j 3.1.0  Copyright 2013 Emitrom LLC
...
//  private static final JavaScriptObject CALLBACKS = getOrCreateCallbacksObject();
  private static final JavaScriptObject CALLBACKS = JavaScriptObject.createArray();
   ...
   */
  private static final native JavaScriptObject getOrCreateCallbacksObject() /*-{
  	var d = {};
  	return d;
  }-*/;
  ...
  public RequestBuilder.Method method = RequestBuilder.GET;
  ...
  void send(final String baseUri) {
    ...
    try {
            xmlHttpRequest.open(method.toString(), baseUri);
    } catch (JavaScriptException e) {
    }
    

//    setHeaders(xmlHttpRequest);
    final RequestCallback rq = new RequestCallback() {
		
		@SuppressWarnings("unchecked")
		@Override
		public void onResponseReceived(Request request, Response response) {
			JavaScriptObject j = JSON.parse(response.getText());
			callback.onSuccess((T) j);
		}
    ...
    final Request request = new Request(xmlHttpRequest, 10000, rq);
    ...
    xmlHttpRequest.setOnReadyStateChange(new ReadyStateChangeHandler() {
        public void onReadyStateChange(XMLHttpRequest xhr) {
            if (xhr.getReadyState() == XMLHttpRequest.DONE) {
                ...
                request.fireOnResponseReceived(rq);
            }
        }
    });
